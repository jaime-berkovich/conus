...
N      = 200
steps  = 5000
dx     = 0.01          # finer spatial grid (same as your Hopf example)
dt     = 0.01
rng    = np.random.default_rng(42)

# ─────────────────────────────────────────────────────
# Baseline parameters (all fixed except r_b)
# – tuned for local Hopf-type oscillations
# ─────────────────────────────────────────────────────
...
D = dict(A=0.001, B=0.001)     # equal diffusion → no Turing wavelength bias



# algebraic threshold from your cubic: r_crit = s * b_a
r_crit    = baseline_params['s'] * baseline_params['b_a']
r_factors = [0.1, 1.0, 10, 100]    # sweep below / at / above the threshold
...
for i, ((recA, recB, avgA, avgB, varA, varB), scen) in enumerate(zip(records, scenarios)):
    ...
    # ─── spatial variance (dual-axis, log) ──────────────────────
    axVarA.plot(time, varA, label='Var[A]', color='tab:blue')
    axVarA.set_yscale('log')
    axVarA.set_title("Spatial variance vs time (log–log)")
...
out_path = os.path.join(out_dir, "r_b_sweep_hopf_stats.png")
